...
class Nyaa:
    ...
    def __init__(self, base_url: str = "https://nyaa.si", cache: bool = True, **kwargs: Any) -> None:
        ...
        self.storage = FileStorage(base_path=_get_user_cache_path())
    ...
    def get(self, page: int | str) -> NyaaTorrentPage:
        ...
        with CacheClient(storage=self.storage, **self.kwargs) as client:
            
            nyaa = client.get(url, extensions=self.extensions).raise_for_status()
            info = self._parse_nyaa(nyaa.text)

            # Get the torrent file and convert it to a torf.Torrent object
            torrent_file = client.get(info["torrent_file"], extensions=self.extensions).raise_for_status().content
    ...
    def search(
        ...
        *,
        ...
        with CacheClient(storage=self.storage, **self.kwargs) as client:
            ...
            nyaa = client.get(self.base_url, params=params, extensions=self.extensions).raise_for_status() # type: ignore
            try:
